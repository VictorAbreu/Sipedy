...
public class FornecedorControllerIT {
	...
	public void findAllDeveriaRetornarPageOrdenadoQuandoOrdenadoPorNomeRazao() throws Exception{
		...
		ResultActions result = mockMvc.perform(get("/fornecedores?page=0&size=12&sort=nomeRazao,asc")
		...
		result.andExpect(jsonPath("$.content[0].nomeRazao").value("Auto Silva"));
		result.andExpect(jsonPath("$.content[1].nomeRazao").value("Auto peças carro top"));
		
	}
	
	@Test
	public void updateDeveriaRetornarFornecedorDTOQuandoIdExistir() throws Exception {
		
		String accessToken = tokenUtil.obtainAccessToken(mockMvc, userName, password);
		
		FornecedorDTO productDTO = Factory.createFornecedorDto();
